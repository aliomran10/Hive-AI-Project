...
def is_valid_move(state, old_tile, new_tile):
    ...
    full_move_check = base_move_check and new_tile.is_hive_adjacent(state) and move_does_not_break_hive(state, old_tile) \
...
def move_obeys_queen_by_4(state):
    ...
    if len(queens_on_board) == 2:
        ...
        if state.turn == 7 and type(state.moving_piece) is pieces.Queen and state.moving_piece.color == WHITE_PIECE:
            return True
        elif state.turn == 8 and type(state.moving_piece) is pieces.Queen and state.moving_piece.color == BLACK_PIECE:
            return True
    elif len(queens_on_board) > 0:
        if queens_on_board[0].color == WHITE_PIECE and state.turn == 7:
            return True
        elif queens_on_board[0].color == BLACK_PIECE and state.turn == 7 and type(state.moving_piece) is pieces.Queen:
            return True
        elif queens_on_board[0].color == BLACK_PIECE and state.turn == 8:
            return True
        elif queens_on_board[0].color == WHITE_PIECE and state.turn == 8 and type(state.moving_piece) is pieces.Queen:
            return True

    return False

def is_queen_played(state):
    if state.turn == 7:
        color = WHITE_PIECE
    else:
        color = BLACK_PIECE
...
def game_is_over(state):
    ...
    if white_surrounded and black_surrounded:
        ...
        state.winner = BLACK_PIECE
        return True
    elif black_surrounded:
        state.winner = WHITE_PIECE
        return True
    else:
        return False
...
def placement_is_allowed(state, old_tile, new_tile):
    if new_tile.has_pieces(): return False
...
def move_is_not_blocked_or_jump(state, old_tile, new_tile):
