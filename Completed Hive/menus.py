import pygame as pg
import webbrowser
from settings import WHITE, BLACK, PURPLE, HOVER_COLOR, WHITE_PIECE, BLACK_PIECE, CLEAR, START_COLOR, WIDTH, HEIGHT
...
def end_menu(screen, state, event):
    ...
    if state.winner == WHITE_PIECE:
        wins = title_font.render('White Wins!', True, PURPLE)
    elif state.winner == BLACK_PIECE:
...
def no_move_popup(screen, surface, state, event,):
    ...
    if state.turn == 7:
        no_move = popup_font.render('The white queen must be placed in this move', True, PURPLE)
        turn_skipped = popup_font.render('', True,
                                         PURPLE)
    elif state.turn == 8:
        no_move = popup_font.render('The black queen must be placed in this move', True, PURPLE)
        turn_skipped = popup_font.render('', True,
                                         PURPLE)
    elif state.turn % 2 == 1:
    ...
    if event.type == pg.KEYDOWN:
        if event.key == pg.K_SPACE:
            if state.turn == 7 or state.turn == 8:
                state.close_popup2()
            else:
                state.close_popup()
